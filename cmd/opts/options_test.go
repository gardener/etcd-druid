...
func TestCompleteWithConfigFlag(t *testing.T) {
	...
	g.Expect(cfg.Server.Webhooks.TLSConfig.ServerCertDir).To(Equal("/var/etcd/ssl/webhook-server-tls"))
	...
	g.Expect(cfg.Controllers.Etcd.EtcdMemberConfig.NotReadyThreshold).To(Equal(metav1.Duration{Duration: 7 * time.Minute}))
	g.Expect(cfg.Controllers.Compaction.Enabled).To(BeTrue())
	g.Expect(cfg.Controllers.Compaction.ConcurrentSyncs).To(PointTo(Equal(3)))
	...
	g.Expect(cfg.Controllers.Etcd.EtcdMemberConfig.UnknownThreshold).To(Equal(metav1.Duration{Duration: 1 * time.Minute}))
