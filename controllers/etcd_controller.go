...
func (r *EtcdReconciler) reconcileEtcd(ctx context.Context, logger logr.Logger, etcd *druidv1alpha1.Etcd) (*string, *appsv1.StatefulSet, error) {
	...
	if etcd.Status.Replicas > 1 && etcd.Spec.Replicas&1 == 0 {
...
func (r *EtcdReconciler) addFinalizersToDependantSecrets(ctx context.Context, logger logr.Logger, etcd *druidv1alpha1.Etcd) error {
	...
	if etcd.Spec.Etcd.ClientUrlTLS != nil {
		...
		secrets = append(secrets,
			...
			&etcd.Spec.Etcd.ClientUrlTLS.TLSCASecretRef,
		)
	}
	...
	if etcd.Spec.Etcd.PeerUrlTLS != nil {
		// As the secrets inside TLS field are required, we error in case they are not found.
		secrets = append(secrets,
			&etcd.Spec.Etcd.PeerUrlTLS.ClientTLSSecretRef,
			&etcd.Spec.Etcd.PeerUrlTLS.ServerTLSSecretRef,
			&etcd.Spec.Etcd.PeerUrlTLS.TLSCASecretRef,
		)
	}
