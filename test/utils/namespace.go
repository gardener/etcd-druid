...
// GenerateTestNamespaceNameWithTestCaseName generates a unique namespace name for testing purposes,
// incorporating the test case name into the namespace name.
func GenerateTestNamespaceNameWithTestCaseName(t *testing.T, prefix, tcName string, suffixLength int) string {
	return GenerateTestNamespaceName(t, fmt.Sprintf("%s-%s", prefix, tcName), suffixLength)
}
