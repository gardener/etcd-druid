...
// TestEtcdOpsTaskLifecycle tests the complete lifecycle of an EtcdOpsTask. i.e Admit, Run and Cleanup.
func TestEtcdOpsTaskLifecycle(t *testing.T) {
	tests := []struct {
		name string
		fn   func(t *testing.T, namespace string, reconcilerTestEnv ReconcilerTestEnv)
	}{
		{"Successful Operation", testEtcdOpsTaskSuccessfulLifecycle},
		{"UnSuccessful Operation", testEtcdOpsTaskUnSuccessfulLifecycle},
	}
	g := NewWithT(t)
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
