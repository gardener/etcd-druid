	...
	g.Eventually(func() bool {
		...
		if etcdOpsTask.Status.State != nil && *etcdOpsTask.Status.State == expectedTaskState {
			if etcdOpsTask.Status.LastOperation != nil &&
				etcdOpsTask.Status.LastOperation.Type == expectedPhase &&
				etcdOpsTask.Status.LastOperation.State == expectedOperationState {
					...
					if len(etcdOpsTask.Status.LastErrors) > 0 {
						for _, lastError := range etcdOpsTask.Status.LastErrors {
							if lastError.Code == *expectedErrorCode {
								t.Logf("task correctly reached state %s in phase %s with error code %s: %s", expectedTaskState, expectedPhase, *expectedErrorCode, lastError.Description)
								return true
							}
						}
					}
				} else {
					t.Logf("task correctly reached state %s in phase %s: %s", expectedTaskState, expectedPhase, etcdOpsTask.Status.LastOperation.Description)
					return true
				}
			}
		}
...
func assertEtcdOpsTaskDeletedAfterTTL(ctx context.Context, g *WithT, t *testing.T, cl client.Client, etcdOpsTask *druidv1alpha1.EtcdOpsTask, expectedFinalPhase druidv1alpha1.LastOperationType) {
