	...
	if tlsConfig := etcd.Spec.Backup.TLS; tlsConfig != nil {
		...
		if err := o.client.Get(ctx, types.NamespacedName{Namespace: etcd.Namespace, Name: tlsConfig.TLSCASecretRef.Name}, etcdbrCASecret); err != nil {
			return &task.Result{
				...
				Error:       druiderr.WrapError(err, ErrGetSecret, task.RunOperation, fmt.Sprintf("failed to get etcdbr CA secret %s/%s", etcd.Namespace, tlsConfig.TLSCASecretRef.Name)),
				Completed:   false,
			}
		}
		...
		if !ok {
			return &task.Result{
				...
				Error:       druiderr.WrapError(fmt.Errorf("CA cert data key %q not found in secret %s/%s", dataKey, etcdbrCASecret.Namespace, etcdbrCASecret.Name), ErrGetEtcd, task.RunOperation, "CA cert data key not found in secret"),
				Completed:   true,
			}
		}
		...
		if !caCerts.AppendCertsFromPEM(certData) {
			return &task.Result{
				...
				Error:       druiderr.WrapError(fmt.Errorf("failed to append CA certs from secret %s/%s", etcdbrCASecret.Namespace, etcdbrCASecret.Name), ErrGetEtcd, task.RunOperation, "failed to append CA certs from secret"),
				Completed:   true,
			}
		}
