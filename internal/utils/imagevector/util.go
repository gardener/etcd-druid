...
func checkVersionConstraint(constraint, version *string) (score int, ok bool, err error) {
	...
	matches, err := versionutil.CheckVersionMeetsConstraint(*version, *constraint)
	if err != nil || !matches {
		return 0, false, err
	}
...
func checkArchitectureConstraint(source []string, desired *string) (score int, ok bool) {
	...
	var sourceArch, desiredArch = []string{ArchitectureAMD64, ArchitectureARM64}, ArchitectureAMD64
