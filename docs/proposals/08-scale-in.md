...
- Preventing removed members from silently rejoining by adding a [membership removal check](#membership-removal-check) in `etcd-backup-restore`.
...
Without a check, `etcd-backup-restore` interprets the state *"this pod has local etcd data, but its member ID is not present in the live cluster"* as a scale-out case and re-adds the removed member as a learner. The controller removes it again, the same pod re-adds itself, and the cluster enters a remove/re-add loop.

##### Proposed solution

`etcd-backup-restore` adds a startup check before the learner-add path. The check uses the presence of the WAL directory (`<data-dir>/member/wal/`) to distinguish a member that ran on this PV before from a pod that started on a fresh PVC. Only if prior etcd data is detected does the check run to verify the member was not explicitly removed.

etcd records removed member IDs in the local boltdb `members_removed` bucket when `MemberRemove` is applied. `etcd-backup-restore` uses the local member's own tombstone to detect that the data directory belongs to a member that was explicitly removed from this cluster; in that case, it must not treat the missing live membership entry as a scale-out/re-add case.

The startup flow after `WasMemberInCluster` returns false is:

1. **WAL directory check** — if `<data-dir>/member/wal/` does not exist, this is a fresh PVC with no prior etcd data; skip directly to the scale-up check.
2. **Membership removal check** (`EnsureNotPermanentlyRemoved`) — if the WAL directory exists, etcd ran on this PV before. The check resolves the local member ID in priority order:
   - **Member lease** — reads the k8s lease holder identity (`<memberID-hex>:<clusterID-hex>:<role>`).
   - **Member-id file** (`<data-dir>/member-id`) — fallback if the lease has no holder identity.
   - If neither yields an ID, the check skips (treats as fresh member).
3. **Scale-up check** (`IsClusterScaledUp`) — runs in both branches (WAL absent and check passed). If the member is not in the live cluster, it is added as a learner.
...
flowchart TD
    ...
    Heartbeat -->|No| WAL{WAL dir exists?\nmember/wal/}
    WAL -->|No| ScaleUp{IsClusterScaledUp?}
    WAL -->|Yes| Lease{Member lease has ID?}
...
- If the **WAL directory is absent** (fresh PVC), there is no prior etcd data to inspect; the check is skipped entirely.
- If **no local member ID** can be resolved (lease has no holder identity, member-id file absent), the member is treated as a fresh join and normal initialization continues.
...
The check opens boltdb **read-only** and reads only the small `members_removed` bucket, so the runtime and memory overhead is negligible. This follows the same access pattern already used by `etcd-backup-restore`'s data validator.

##### Limitation

The check relies on the WAL directory as the gate and the `members_removed` tombstone for the decision. If the data directory is wiped (PVC deleted and recreated during scale-out), the WAL directory is absent and the check is skipped — the member is treated as a fresh join and added as a learner. This case is outside the check's reach by design. The backstop here is the `etcd-druid` controller: scale-in detection re-derives the target set on every reconcile and removes the surplus member again under the per-cycle quorum-safety check, so a member restarting with a wiped data directory cannot persist in the cluster after a scale-in.

The check does not interfere with the normal single-member restoration path. It runs only when `WasMemberInCluster` is false and the WAL directory exists — i.e. the member is absent from the live cluster but has prior data on the PV. A valid member whose data directory is merely corrupted is **still a member** of the cluster (`WasMemberInCluster=true`), so the check is skipped and the existing restoration flow runs unchanged. Single-node clusters are a [Non-Goal](#non-goals) and the check is gated on multi-node in any case.
...
- [`bucket.go` — `MembersRemoved` bucket definition](https://github.com/etcd-io/etcd/blob/62d8759b7d5dbc9b3694f89d54170a55726bb485/server/mvcc/buckets/bucket.go#L31-L49)
